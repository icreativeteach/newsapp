<div class="sw-card">
	<div class="sw-card__title">
          
    </div>
    <div class="sw-card__content">
    	<div class="sw-container">
			<div class="sw-page">
				<div class="sw-page__head-area">
					<div class="sw-page__search-bar">
						<div class="sw-search-bar">
							<div class="">
								<div class="sw-search-bar__field">
									 
				            		<input placeholder="Search newsletter..." class="sw-search-bar__input"> 
				            	 
				            		<span class="sw-search-bar__field-icon sw-search-bar__field-search-icon">
				            			<span class="sw-icon icon--small-search sw-icon--small">
				            				<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><path fill="#758CA3" fill-rule="evenodd" d="M6.52346564,10.8907479 L2.20710678,15.2071068 C1.81658249,15.5976311 1.18341751,15.5976311 0.792893219,15.2071068 C0.402368927,14.8165825 0.402368927,14.1834175 0.792893219,13.7928932 L5.10925208,9.47653436 C4.41079097,8.49571549 4,7.29583043 4,6 C4,2.6862915 6.6862915,0 10,0 C13.3137085,0 16,2.6862915 16,6 C16,9.3137085 13.3137085,12 10,12 C8.70416957,12 7.50428451,11.589209 6.52346564,10.8907479 Z M10,10 C12.209139,10 14,8.209139 14,6 C14,3.790861 12.209139,2 10,2 C7.790861,2 6,3.790861 6,6 C6,8.209139 7.790861,10 10,10 Z"></path>
				            				</svg>
				            			</span>
				            		</span>
				            	</div> 
				            </div> 
				        </div>
				    </div> 
				  
				  	<div class="smart-bar__content">
					  	<div class="smart-bar__header">
					  		<h2>Newsletter
					  			<span class="sw-page__smart-bar-amount">(1)</span>
					  		</h2>
					  	</div>
					  	<div class="smart-bar__actions">
					  		<div class="sw-tooltip--wrapper">
					  			<a href="#" class="sw-button sw-button--primary" tooltip-id="ce042f48891944e9bd1b5ad93838175b">
						  			<span class="sw-button__content">
						            	Add newsletter
						        	</span>
						        </a>
						    </div>
						</div>
					</div>
				</div>
				<div class="sw-page__content">
					<div class="sw-page__main-content">
						<div class="sw-page__main-content-inner">
							<div class="sw-product-list__content">
								<div class="sw-data-grid">
									<div class="sw-data-grid__wrapper">
										<table class="sw-data-grid__table">
											<thead class="sw-data-grid__header">
												<tr class="sw-data-grid__row">
													<th class="sw-data-grid__cell sw-data-grid__cell--selection">
														<div class="sw-data-grid__cell-content">
															<div class="sw-field--checkbox sw-data-grid__select-all">
																<div class="sw-field--checkbox__content">
																	<div class="sw-field__checkbox">
																		<input id="sw-field--ad43f4afc064445582b69eb1f9265c3a" type="checkbox" name="sw-field--ad43f4afc064445582b69eb1f9265c3a"> 
																		<div class="sw-field__checkbox-state"><span class="sw-icon icon--small-default-checkmark-line-small sw-icon--fill" style="width: 16px; height: 16px;"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" viewBox="0 0 16 16"><defs><path id="icons-small-default-checkmark-line-small-a" d="M5.70710678,7.29289322 C5.31658249,6.90236893 4.68341751,6.90236893 4.29289322,7.29289322 C3.90236893,7.68341751 3.90236893,8.31658249 4.29289322,8.70710678 L6.29289322,10.7071068 C6.68341751,11.0976311 7.31658249,11.0976311 7.70710678,10.7071068 L11.7071068,6.70710678 C12.0976311,6.31658249 12.0976311,5.68341751 11.7071068,5.29289322 C11.3165825,4.90236893 10.6834175,4.90236893 10.2928932,5.29289322 L7,8.58578644 L5.70710678,7.29289322 Z"></path></defs><use fill="#758CA3" fill-rule="evenodd" xlink:href="#icons-small-default-checkmark-line-small-a"></use></svg></span></div>
																		</div> 
																		<div class="sw-field"></div>
																</div> 
															</div>
														</div>
													</th> 
													<th class="sw-data-grid__cell">
														<div class="sw-data-grid__cell-content">
															Name
                               							</div> 
                               						</th>
                               						<th class="sw-data-grid__cell">
                               							<div class="sw-data-grid__cell-content">
                                    						Product number
                                  						</div>
                                  					</th>
                                  					<th class="sw-data-grid__cell">
                                  						<div class="sw-data-grid__cell-content">
                                    						Manufacturer
                                  						</div>
                                  					</th>
                                  					<th class="sw-data-grid__cell">
                                  						<div class="sw-data-grid__cell-content">
                                    						Active
                                 						</div> 
                                 					</th>
                                 					<th class="sw-data-grid__cell">
                                 						<div class="sw-data-grid__cell-content">
                                    						Euro
                                  						</div>
                                  					</th>
                                  					<th class="sw-data-grid__cell">
                                  						<div class="sw-data-grid__cell-content">
                                    						In stock
                                 						</div> 
                                 					</th>
                                 					<th class="sw-data-grid__cell">
                                 						<div class="sw-data-grid__cell-content">
                                    						Available
                        								</div>
                        							</th> 
                        							<th aria-hidden="true" class="sw-data-grid__cell sw-data-grid__cell-spacer">
                        								<div class="sw-data-grid__cell-content">
                        								</div>
                        							</th> 
                        							<th class="sw-data-grid__cell sw-data-grid__cell--header sw-data-grid__cell--actions">
                        								<div class="sw-data-grid__cell-content">
                        									<div class="sw-context-button" tooltip-id="ee6b0b70ec7049a3aef41efeb2cba711">
                        										<button aria-label="List settings" class="sw-button sw-data-grid-settings__trigger sw-button--x-small sw-button--square">
                        											<span class="sw-button__content">
                        												<span class="sw-icon icon--small-default-stack-line sw-icon--fill" style="width: 14px; height: 14px;">
                        													<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                        														<path fill="#758CA3" fill-rule="evenodd" d="M2,4 C1.44771525,4 1,3.55228475 1,3 C1,2.44771525 1.44771525,2 2,2 L14,2 C14.5522847,2 15,2.44771525 15,3 C15,3.55228475 14.5522847,4 14,4 L2,4 Z M2,9 C1.44771525,9 1,8.55228475 1,8 C1,7.44771525 1.44771525,7 2,7 L14,7 C14.5522847,7 15,7.44771525 15,8 C15,8.55228475 14.5522847,9 14,9 L2,9 Z M2,14 C1.44771525,14 1,13.5522847 1,13 C1,12.4477153 1.44771525,12 2,12 L14,12 C14.5522847,12 15,12.4477153 15,13 C15,13.5522847 14.5522847,14 14,14 L2,14 Z"></path>
                        													</svg>
                        												</span>
                        											</span>
                        										</button> 
                        									</div> 
                        								</div>
                        							</th>
                        						</tr>
                        					</thead> 
                        					<tbody class="sw-data-grid__body">
                        						<tr class="sw-data-grid__row sw-data-grid__row--0">
                        							<td class="sw-data-grid__cell sw-data-grid__cell--selection">
                        								<div class="sw-data-grid__cell-content">
                        									<div class="sw-field--checkbox">
                        										<div class="sw-field--checkbox__content">
                        											<div class="sw-field__checkbox">
                        												<input id="sw-field--74241a753d914b37a72a0f33800826b2" type="checkbox" name="sw-field--74241a753d914b37a72a0f33800826b2"> 
                        												<div class="sw-field__checkbox-state">
                        													<span class="sw-icon icon--small-default-checkmark-line-small sw-icon--fill" style="width: 16px; height: 16px;">
                        														<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" viewBox="0 0 16 16"><defs><path id="icons-small-default-checkmark-line-small-a" d="M5.70710678,7.29289322 C5.31658249,6.90236893 4.68341751,6.90236893 4.29289322,7.29289322 C3.90236893,7.68341751 3.90236893,8.31658249 4.29289322,8.70710678 L6.29289322,10.7071068 C6.68341751,11.0976311 7.31658249,11.0976311 7.70710678,10.7071068 L11.7071068,6.70710678 C12.0976311,6.31658249 12.0976311,5.68341751 11.7071068,5.29289322 C11.3165825,4.90236893 10.6834175,4.90236893 10.2928932,5.29289322 L7,8.58578644 L5.70710678,7.29289322 Z"></path></defs><use fill="#758CA3" fill-rule="evenodd" xlink:href="#icons-small-default-checkmark-line-small-a"></use>
                        														</svg>
                        													</span>
                        												</div>
                        											</div> 
                        										</div>
                        									</div>
                        								</td> 
                        								<td class="sw-data-grid__cell">
                        									<div class="sw-data-grid__cell-content"> 
                        										<div>
                        											<a href="#/sw/product/detail/c95cce60de474cbea135422fd4094985" class="">
                            											sample 1
                        											</a>
                        										</div>
                        									</div>
                        								</td>
                        								<td class="sw-data-grid__cell">
                        									<div class="sw-data-grid__cell-content"> 
                        										<span class="sw-data-grid__cell-value">
                                            						SW10000
                                        						</span>
                                        					</div>
                                        				</td><td class="sw-data-grid__cell"><div class="sw-data-grid__cell-content"> <span class="sw-data-grid__cell-value">
                                            abc
                                        </span></div></td><td class="sw-data-grid__cell sw-data-grid__cell--active sw-data-grid__cell--align-center"><div class="sw-data-grid__cell-content"> <span class="is--active sw-icon icon--small-default-checkmark-line-medium sw-icon--fill sw-icon--small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><path fill="#758CA3" fill-rule="evenodd" d="M3.70710678,8.29289322 C3.31658249,7.90236893 2.68341751,7.90236893 2.29289322,8.29289322 C1.90236893,8.68341751 1.90236893,9.31658249 2.29289322,9.70710678 L5.29289322,12.7071068 C5.68341751,13.0976311 6.31658249,13.0976311 6.70710678,12.7071068 L13.7071068,5.70710678 C14.0976311,5.31658249 14.0976311,4.68341751 13.7071068,4.29289322 C13.3165825,3.90236893 12.6834175,3.90236893 12.2928932,4.29289322 L6,10.5857864 L3.70710678,8.29289322 Z"></path></svg></span></div></td><td class="sw-data-grid__cell sw-data-grid__cell--price-EUR sw-data-grid__cell--align-right"><div class="sw-data-grid__cell-content"> 
                    €45.00
                </div></td><td class="sw-data-grid__cell sw-data-grid__cell--stock sw-data-grid__cell--align-right"><div class="sw-data-grid__cell-content"> 
                        10
                        <span class="sw-color-badge is--warning"></span></div></td><td class="sw-data-grid__cell"><div class="sw-data-grid__cell-content"> <span class="sw-data-grid__cell-value">
                                            10
                        </span>
                    </div>
                </td> 
                <td aria-hidden="true" class="sw-data-grid__cell sw-data-grid__cell-spacer"><div class="sw-data-grid__cell-content"></div></td> <td class="sw-data-grid__cell sw-data-grid__cell--actions"><div class="sw-data-grid__cell-content"><div class="sw-context-button sw-data-grid__actions-menu"><button class="sw-context-button__button"><span aria-hidden="true" class="sw-icon icon--small-more sw-icon--fill sw-icon--small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><path fill="#758CA3" fill-rule="evenodd" d="M3.5,9.5 C2.67157288,9.5 2,8.82842712 2,8 C2,7.17157288 2.67157288,6.5 3.5,6.5 C4.32842712,6.5 5,7.17157288 5,8 C5,8.82842712 4.32842712,9.5 3.5,9.5 Z M8,9.5 C7.17157288,9.5 6.5,8.82842712 6.5,8 C6.5,7.17157288 7.17157288,6.5 8,6.5 C8.82842712,6.5 9.5,7.17157288 9.5,8 C9.5,8.82842712 8.82842712,9.5 8,9.5 Z M12.5,9.5 C11.6715729,9.5 11,8.82842712 11,8 C11,7.17157288 11.6715729,6.5 12.5,6.5 C13.3284271,6.5 14,7.17157288 14,8 C14,8.82842712 13.3284271,9.5 12.5,9.5 Z"></path></svg></span></button> <!----></div> <!----></div></td></tr></tbody></table></div> </div> </div></div> </div>
			</div>
			<div class="sw-context-menu" style="width: 220px;">
			   <div class="sw-context-menu__content">
			      <div class="sw-tooltip--wrapper">
			         <a href="#" class="sw-context-menu-item sw-entity-listing__context-menu-edit-action>
			           <span class="sw-context-menu-item__text">
			            Edit
			            </span>
			         </a>
			      </div>
			      <div class="sw-context-menu-item sw-mail-header-footer-list-grid__duplicate-action">
			          <span class="sw-context-menu-item__text">
			         Duplicate
			         </span>
			      </div>
			      <div class="sw-tooltip--wrapper">
			         <div class="sw-context-menu-item sw-entity-listing__context-menu-edit-delete sw-context-menu-item--danger">
			            <span class="sw-context-menu-item__text">
			            Delete
			            </span>
			         </div>
			      </div>
			   </div>
			</div>
       	</div>
    </div> 
</div>
</div>
